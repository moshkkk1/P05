...
void move_ball(void) {
    ...
    else if (x_ball == WIDTH - 2 && y_ball >= y_pad2 && y_ball < y_pad2 + PADDLE_SIZE) {
        dx = -1;
...
void draw_playground(void) {
    ...
    mvprintw(1, (WIDTH - 25) / 2, "Player 1: %d  |  Player 2: %d", score1, score2);
    ...
    for (int i = 3; i < HEIGHT + 2; i++) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            if (i == 3 || i == HEIGHT + 1) {
                ...
                else if (j == WIDTH - 2 && i - 3 >= y_pad2 && i - 3 < y_pad2 + PADDLE_SIZE) {
                    mvaddch(i, j, '|');
                }
                // Мяч
