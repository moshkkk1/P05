...
/*
Инициализация ракеток, мяча и очков. В ракетках мы инициализируем верхнюю черту, а середину и дно будем
рисовать уже в самом поле. Цифры не рандомные, а научно натыканные, чтоб ракетки стояли в середине.
*/
void init(void) {
    y_pad1 = (HEIGHT - 3) / 2;
    y_pad2 = (HEIGHT - 3) / 2;
...
void move_ball(void) {
    ...
    /*
    Отскок от ракеток. Тут подробнее. Ракетка имеет три секции. Если мяч прилетел в верхнюю часть ракетки, то
    и отскок будет вверх, если в нижнюю часть, то вниз. Это добавлено специально, ибо без данного функционала,
    игра превращалась в бесконечный алгоритм повторяющихся действий. А так игра получает хоть небольшую дозу
    неопределенности.
    */
    // Левая ракетка
